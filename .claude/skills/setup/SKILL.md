---
name: setup
description: Set up the local development environment for new contributors — installs Python via pyenv, configures Poetry, and runs setup.sh
user-invocable: true
...
2. Check prerequisites (`pyenv`, `poetry`) and ensure Poetry is version `2.3.4`
...
Extract the **install target** as the lower bound major.minor (e.g. `3.14`). Use this version for all subsequent steps.
...
This project requires *Poetry 2.3.4* exactly. If the installed version differs, update it:

```bash
poetry self update 2.3.4
```

Validate again with `poetry --version` and confirm it shows `2.3.4` before continuing.
...
The output must match the target version (e.g. *Python 3.14.x*). If not, stop and diagnose.

---

### Step 3 — Create the virtualenv manually with pyenv's Python

**Do NOT use `poetry env use`.** In Poetry `2.3.4`, that command validates the system Python
against the project constraint — if the system has a Python outside the constraint range
(e.g. Homebrew Python 3.14), it rejects the command even when you pass a valid Python path.
...
The output must match the target version (e.g. `Python 3.14.4`). Stop and diagnose if not.
...
### Step 6 — Run setup_local.sh

```bash
sh commands/setup_local.sh
```
...
to bypass the system Python constraint check bug in Poetry `2.3.4`.
...
- **Non-blocking failures** (`setup.sh`) should be noted but must not abort the overall setup;
