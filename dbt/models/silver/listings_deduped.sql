...
WITH SOURCE AS (
    ...
    WHERE
        DATE(SRC_EXECUTED_AT_TS) = {% if file_date %}DATE('{{ file_date }}'){% else %}CURRENT_DATE{% endif %}  -- noqa: LT05
...
SELECT
    ...
    COALESCE(LISTING_NUMBER_OF_ROOMS_AMT, {{ default_int }}) AS ROOMS,
    COALESCE(LISTING_NUMBER_OF_BEDROOMS_AMT, {{ default_int }}) AS BEDROOMS,
    COALESCE(LISTING_NUMBER_OF_BATHROOMS_AMT, {{ default_int }}) AS BATHROOMS,
    ...
    COALESCE(LISTING_OFFERS_INFOS.PROPERTY_VALUE.VALUE, {{ default_int }}) AS OFFERS_PRICE,
    COALESCE(
    ...
    SRC_SITE_NAME AS SITE,
    SRC_CITY_NAME AS CITY,
    SRC_PROPERTY_TYPE_CAT AS PROPERTY_TYPE,
...
WHERE LISTING_URL IS NOT NULL
    AND LISTING_FLOOR_SIZE_INFOS.VALUE IS NOT NULL
    AND LISTING_OFFERS_INFOS.PRICE IS NOT NULL
QUALIFY
    ROW_NUMBER() OVER (
        PARTITION BY
            SCRAPE_DATE,
