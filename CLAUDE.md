...
- **Branch prefixes** are independent from commit prefixes (lowercase letters and digits, hyphen-separated, ≤ 3 segments after the `/`). Allowed prefixes depend on the PR target, enforced by the `branch-checks` job in `.github/workflows/pr-checks.yml`:
  - → `develop`: `enhancement/*`, `feature/*`, `fix/*`, `hotfix/*`, `refactor/*`, `release/*`; `backport/*` is also allowed but is auto-generated by `.github/workflows/backport.yml` on every push to `main` — do not create `backport/*` branches manually; they are exempt from the slug convention check
